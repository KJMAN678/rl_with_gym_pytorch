...
class ReplayBuffer:
    ...
    def sample(self, batch_size):
        ...
        samples = [slef.buffer[i] for i in idxs]
    ...
    def td_loss_dqn(
        ...
        next_states = torch.tensor(
            np.array(next_states), device=DEVICE, dtype=torch.float
        )
...
class BitFlipEnvironment:
    ...
    def reset(self):
        ...
        self.goal = np.random.randint(2, size=self.bits).astype(np.flaot32)
    ...
    def step(self, action):
        ...
        reward, done = self.compute_reward(self.state, self.gaol)
